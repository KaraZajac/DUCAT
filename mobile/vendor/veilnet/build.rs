...
fn main() {
    ...
    if is_input_file_outdated("./proto/veilnet.capnp", "./src/proto/veilnet_capnp.rs").unwrap() {
        // Compile Capn'Proto
        ::capnpc::CompilerCommand::new()
            .output_path("src/")
            .default_parent_module(vec!["proto".into()])
            .file("proto/veilnet.capnp")
            .run()
            .expect("compiling schema");
        // If successful, append a hash of the input to the output file
        append_hash("proto/veilnet.capnp", "src/proto/veilnet_capnp.rs");
    }
}
