...
const LISTENER_RESTART_AFTER_CONSECUTIVE_ERRORS: usize = 16;
...
impl NativeNetwork {
    ...
    async fn tcp_acceptor(
        ...
        consecutive_accept_errors: Arc<AtomicUsize>,
        ...
        let tcp_stream = match tcp_stream {
            Ok(v) => {
                consecutive_accept_errors.store(0, Ordering::Relaxed);
                v
            }
            Err(e) => {
                let listener_addr = listener_state.read().addr;
                let n = consecutive_accept_errors.fetch_add(1, Ordering::Relaxed) + 1;
                if n >= LISTENER_RESTART_AFTER_CONSECUTIVE_ERRORS {
                    veilid_log!(self debug
                        "TCP acceptor on {} errored {} times consecutively ({}), triggering network restart",
                        listener_addr, n, e);
                    self.inner.lock().network_needs_restart = true;
                } else {
                    veilid_log!(self debug
                        "TCP acceptor on {} transient error {}/{} ({})",
                        listener_addr, n, LISTENER_RESTART_AFTER_CONSECUTIVE_ERRORS, e);
                }
                return;
            }
        };
        ...
        // Hold the per-remote address lock from accept time so a concurrent outbound waits and reuses
        let Some(address_lock_guard) = connection_manager
            .acquire_inbound_address_lock(peer_addr)
            .await
        else {
            veilid_log!(self debug "timed out acquiring inbound address lock for {}", peer_addr);
            return;
        };
        ...
        if let Err(e) = connection_manager
            .on_accepted_protocol_network_connection(conn, address_lock_guard)
            .await
        {
    ...
    #[cfg_attr(feature = "instrument", instrument(level = "trace", skip_all, fields(__VEILID_LOG_KEY = self.log_key())))]
    fn spawn_socket_listener(&self, addr: SocketAddr) -> EyreResult<bool> {
        // Get config
        ...
        let listener_state = Arc::new(RwLock::new(ListenerState::new(addr)));
        self.inner
            .lock()
            .listener_states
        ...
        let consecutive_accept_errors = Arc::new(AtomicUsize::new(0));
        ...
        let jh = spawn(&format!("TCP listener {}", addr), async move {
            ...
            let _ = incoming_stream
                .for_each_concurrent(None, |tcp_stream| {
                    ...
                    let listener_state = listener_state.clone();
                    let connection_manager = connection_manager.clone();
                    let consecutive_accept_errors = consecutive_accept_errors.clone();
                    Self::tcp_acceptor(
                        this,
                        tcp_stream,
    ...
    #[cfg_attr(feature = "instrument", instrument(level = "trace", skip_all, fields(__VEILID_LOG_KEY = self.log_key())))]
    pub(super) fn start_tcp_listener(
        &self,
        ...
        for ip_addr in bind_set.addrs {
            ...
            loop {
                ...
                if !self.inner.lock().listener_states.contains_key(&addr) {
                    if self.clone().spawn_socket_listener(addr)? {
                        got_listener = true;
                    }
                } else {
                    got_listener = true;
                ...
                if got_listener {
                    let ls = if let Some(ls) = self.inner.lock().listener_states.get_mut(&addr) {
                        ...
                        bail!("listener state missing for {} after binding", addr);
                    };
