...
pub(super) const EXTERNAL_INFO_VALIDATIONS: usize = 5;
...
impl DiscoveryContext {
    pub fn new(
        ...
        let intf_addrs = network
            .last_network_state()
            .unwrap_or_log()
            .interface_address_state
            .interface_addresses
            .iter()
            .map(|x| Address::from_ip_addr(x.ip()))
            .collect();
    ...
    async fn request_public_address(&self, node_ref: FilteredNodeRef) -> Option<SocketAddress> {
        ...
        let answer = match Box::pin(
            rpc.rpc_call_status(Destination::direct(node_ref.clone(), None)),
        )
        .await
        {
            Ok(StatusResult::Answer { answer, .. }) => answer,
            Ok(other) => {
        ...
        answer.answer.opt_sender_info.map(|si| si.socket_address)
    }
    ...
    async fn discover_external_addresses(&self) -> bool {
        ...
        let inbound_dial_info_entry_filter = self
            .routing_table()
            .make_inbound_dial_info_entry_filter(routing_domain, dial_info_filter);
        let disallow_relays_filter = Box::new(
            move |opt_snap: &Option<BucketEntrySnapshot>, _cur_ts: Timestamp| {
                ...
                if let Some(pi) = snap.get_peer_info(routing_domain) {
                    pi.node_info().relay_ids().is_empty()
                } else {
                    false
                }
            },
                ...
                snap.get_peer_info(routing_domain)
                    .map(|pi| {
                        pi.node_info()
                            ...
                            && pi.node_info().is_fully_direct_inbound()
                    })
                    .unwrap_or(false)
            },
        ) as RoutingTableEntryFilter;
        ...
        let get_public_address_func = |node: NodeRef| {
            ...
            async move {
                if let Some(address) = Box::pin(this.request_public_address(node.clone())).await {
                    let dial_info = this.network.make_dial_info(address, protocol_type);
        ...
        for node in nodes.iter().cloned() {
            ...
            if unord.len() == EXTERNAL_INFO_CONCURRENCY {
                    ...
                    .await
                {
                    ...
                    Ok(None) => {
                        // unord just pushed; next() yielding None here should be impossible, skip
                        veilid_log!(self error "discover_external_addresses: empty unord poll");
                    }
                    Err(_) => {
                        // stop requested
                        return false;
        ...
        external_address_infos.sort_unstable_by(|a, b| {
    ...
    async fn try_upnp_port_mapping(&self) -> Option<DialInfo> {
        ...
        let igd_manager = self.network.igd_manager.clone()?;
        ...
        let external_mapped_dial_info = self.network.make_dial_info(
    ...
    fn protocol_process_mapped_dial_info(
        ...
        for external_info in external_infos {
            ...
            let do_map_validate_fut: PinBoxFutureStatic<DetectionResultKind> = Box::pin(
                async move {
                    veilid_log!(this debug "validating mapped dial info {:?} on node {:?}", mapped_dial_info, external_info.node);

                    // Do a validate_dial_info on the external address from a redirected node
                    if this
                        .validate_dial_info(
                            external_info.node.clone(),
                            mapped_dial_info.clone(),
                            true,
                        )
                        .await
                    {
                        veilid_log!(this debug "successfully validated mapped dial info {:?} on node {:?}", mapped_dial_info, external_info.node);
                        ...
                        veilid_log!(this debug "failed to validate mapped dial info {:?} on node {:?}", mapped_dial_info, external_info.node);
            ...
            unord.push(do_map_validate_fut);
        }
    }
    ...
    fn protocol_process_nat(
        ...
        // If any local interface address of this context's family is a carrier-grade NAT or
        // DS-Lite indicator, classify as Symmetric for this family only. Other families may
        // still be globally reachable.
        let family_intf_addrs_indicate_cgnat = self
            .intf_addrs
            .iter()
            .filter(|a| a.address_type() == self.config.address_type)
            .any(|a| a.is_cgnat_indicator());
        if family_intf_addrs_indicate_cgnat {
            let external_address_types =
                external_info.iter().fold(AddressTypeSet::new(), |acc, ei| {
                    acc | ei.address.address_type()
                });
            let this = self.clone();
            let do_cgnat_symmetric_fut: PinBoxFutureStatic<DetectionResultKind> = Box::pin(
                async move {
                    veilid_log!(this debug
                        "Local interface address indicates carrier-grade NAT or DS-Lite; classifying as SymmetricNAT for {:?}:{:?}",
                        this.config.protocol_type, this.config.address_type
                    );
                    DetectionResultKind::Result {
                        possibilities: vec![],
                        result: DetectionResult {
            ...
            unord.push(do_cgnat_symmetric_fut);
            return;
        }
        ...
        let retry_count = self.config().internal().network.restricted_nat_retries;
    ...
    pub async fn discover(self) -> Option<DetectionResult> {
        ...
        ///////////
        // igd_manager is only present when UPnP is enabled
        if self.network.igd_manager.is_some() {
