    ...
    pub async fn rpc_call_validate_dial_info(
        ...
        let validate_dial_info_receipt_time_ms =
            self.config()
                ...
                .network
                .dht
                .validate_dial_info_receipt_time_ms as u64;
        ...
        let (receipt, eventual_value) = network_manager
            .generate_single_shot_receipt(receipt_time, Bytes::new())
            .await
            .map_err(RPCError::internal)?;

        // Use the same address family to send the validate_dial_info request because when we call this, we
        // are still figuring out what address families are supported by this node
        let node_ref_filter = NodeRefFilter::new().with_address_type(dial_info.address_type());

        // Build the statement to send
        ...
        network_result_value_or_log!(self self.statement(Destination::direct(
            peer.custom_filtered(node_ref_filter).with_sequencing(Sequencing::PreferOrdered),
    ...
    pub(super) async fn process_validate_dial_info(&self, msg: Message) -> RPCNetworkResult<()> {
        ...
        let (dial_info, receipt, redirect) = match kind {
            RPCOperationKind::Statement(s) => match s.destructure() {
                ...
                _ => return Ok(NetworkResult::invalid_message("not a validate dial info")),
            },
        ...
        if redirect {
            ...
            let outbound_dial_info_entry_filter = routing_table
                .make_outbound_dial_info_entry_filter(routing_domain, dial_info.clone());
            ...
            for peer in peers {
                ...
                // Send the validate_dial_info request
                // This can only be sent directly, as relays can not validate dial info
                network_result_value_or_log!(self pin_future_closure!(self.statement(Destination::direct(peer.sequencing_filtered(Sequencing::PreferUnordered), None), statement, None, None))
                    .await? => [ format!(": peer={} statement={:?}", peer, statement) ] {
                        continue;
                    }
                );
