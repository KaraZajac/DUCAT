...
| 3.3 | Burn: send to the DUCAT burn address from the wallet, keep the proof in the persona's attestation record and the backup. **Both clients burn and keep the proof (2026-09-14)**; the desk carries burns, verified burns and receipts in the backup as `burns_raw` / `verified_burns_raw` / `attestations_{given,received,about}_raw` under the phone's JSON names (round-trip tested); the phone writes and reads the same five entries through `Trust.backupEntries` / `Trust.restoreEntry`. **Restore-then-verify walked on the phone 2026-09-14**: the envelope after the restore is byte-identical to the one the checker verified. | a burn on stagenet, restored from a backup, still verifies |

### Phase 4 — the clients

| Step | What | Proof |
|---|---|---|
| 4.1 | Burn from the wallet screen, the cost said plainly and irreversibly; the proof stored and carried. Both clients. **Desk done 2026-09-14** (Wallet → Burn, the first real stagenet burn); **phone done 2026-09-14** (`Trust.kt`, `ui/Burn.kt` off the money screen, PIN between the button and the send, the proof made at send time, the envelope signed on the poller's sweep once the block is known via the bridge's `burn_proof_sign`; memo `research/security/phase4-phone-burn.md`). **Walked on an Android build 2026-09-14** (`research/security/phase4-phone-walk.md`, the x86_64 debug APK on the `ducat` AVD against headless desks): the phone burned 0.01 XMR, the sweep signed the envelope one second after the block was seen, a desk that had never met the phone verified it in one second, and the envelope survived an uninstall and a restore from an encrypted backup byte for byte. Walked desk↔desk on the live chain the same day (`app/examples/mailbox.rs` `checker` / `burner <card>`): the bank desk burned 0.01 XMR under its persona (txid `c7c973fa…c362`), made the out-proof at send time, signed the `BURN_PROOF` once the block came (2207352, 208 s later), showed it as a `ducat:burn/` link; a fresh checker desk opened it, verified the proof against its own node for exactly 0.01 XMR, got the block from a second node, and wore it as "burned 0.010000 XMR, since block 2207352" — 12 s from proof to verdict. The walk found and fixed a real bug: a desk that had never used its wallet had no node and refused to check anything. | walk on phone and desk |
| 4.2 | The badge on listings, hails, tills and in the thread; the gate; "ask for their record" in a thread, answered from the attestation record; a seller's minimum. **Desk, thread part, done 2026-09-14**: the badge in the thread header ("burned 0.01 XMR, since block N · 3 receipts, 1 from burned personas · 4.7 ★"), "Show my burn" / "Check" on a `ducat:burn/` bubble, the gate warning in the pay form whenever the amount exceeds the counterparty's verified burn. **Listing and till, desk, done 2026-09-14**: an open listing says what this desk knows about its poster ("burned … since block N · receipts" or "No burn of theirs checked here"); the till's customer picker carries the same words on each contact. Hails are not on the desk. **Phone, thread part, done 2026-09-14** (`Trust.kt`, `ui/Chat.kt`, `ui/Pay.kt`; memo `research/security/phase4-phone-trust.md`): the two-line header badge, Show my burn / Show my record / Rate them in the tray, Check on a burn bubble, the gate under the amount in Pay (a send to a contact only). **Phone walk 2026-09-14**: "Show my burn" and the receipt bubbles read as the memo said; the badge stayed empty where it should (the desk's verdict is the desk's). **Seller's minimum, wire and desk, done 2026-09-14** (draft dev12: `RENTAL_NOTICE` field 320 `min_burn`, written only when there is one, one vector; the desk's listing form takes it, an open listing says "The seller asks that buyers have burned at least X" and warns the buyer whose own burn falls short — before anything is committed, as §9.5 requires). **Phone, minimum and the badge outside the thread, done 2026-09-14** (memo `research/security/phase4-phone-minimum.md`): the poster's form takes the minimum, a found listing says what the seller asks and warns a buyer whose own burn falls short above Ask, never disabling it; the badge in words on a found listing's poster, on the bar tab's regulars and on the ride offer. On both clients a listing's poster key is per-listing, so its badge reads "No burn of theirs checked here" until the card is claimed — the real badge lives in the enquiry thread's header. The desk's "yours" on a found listing was never true (it compared the per-listing key with personas); it now matches the card this desk minted, as the phone does. Group threads do not ingest trust links, by decision: a group page is sealed under the group key and attributes its entries by subkey, and §9.5 says a proof travels in a sealed 1:1 message — a group is not where a record is shown. **Marketplace walk done 2026-09-14** (`research/security/phase4-phone-market-walk.md`): an unburned phone found a desk's listing and read "No burn of theirs checked here · The seller asks that buyers have burned at least 0.010000 XMR · This persona has burned nothing yet. They may not deal with you.", with Ask still enabled; the Pay gate read "They have burned nothing you have checked. If they vanish, nothing holds them." and the payment went through with the warning standing (0.005 XMR, stagenet block 2207762). One gap it found is closed: the bill screen now wears the badge and the send confirmation carries the gate sentence, so §9.5's words stand at every decision on the phone. The other is a decision for a person: **a listing's card carries no payout address**, so a buyer who obeys the minimum arrives at a dark Pay button and is told to switch to Request. Putting the seller's address in subkey 0 would publish it to every reader of the board — the leak W4 just closed for the claimant — so the answer is either an address exchanged in the sealed thread after the claim, or a plainer sentence telling the buyer to ask for a bill. | marketplace walk with a burned seller and an unburned one |
| 4.3 | Rated receipts after a settled deal, signed to the counterparty's persona, weighted by the signer's own burn; shown on request. **Desk done 2026-09-14** (`app/src/trust.rs`, spec §9.5 "How a receipt travels"): "Rate them" appears once the thread holds a receipt, signs an `ATTESTATION` and sends it as `ducat:attest/`; the subject keeps it only if the sender signed it and it is about one of its personas; "Show my record" sends `ducat:record/<hex>.<hex>…`; a reader keeps what opens and is about the sender and weighs it itself — one voice per signer, rated by the latest, counted only when that signer's burn was verified here. Tests cover the travel, the one-voice rule and the refusals. **Proven live 2026-09-14** desk↔desk (`app/examples/mailbox.rs` `rated` / `rater`): the receipt landed in the rated desk's thread and was kept by the inbox on its own, the record came back, and the rater read it (1 receipt, 0 weighted — no burn verified for the rater, as it should be) in 47 s. **Phone done 2026-09-14**, the same rules (the port found two the desk then adopted: sign under the thread's owner persona, not the worn hat; pack a record link to one 2000-character message, newest first). Eight JVM tests. **Walked phone↔desk 2026-09-14** in the reverse direction: a desk's receipt landed on the phone's record ("A rating for you, kept on your record"), "Show my record" sent it back, the desk read it (1 receipt, 0 weighted). The forward direction waits for a deal that produces a kind-3 receipt. | kiosk and rental walks produce receipts both sides can show |
| 4.4 | Vouching along in-person card edges, computed locally, never published. **Both clients done 2026-09-14** (draft dev12: `VOUCH` type 29, fields 317–319, five vectors; desk `app/src/trust.rs` vouch section, phone `Trust.kt` + `ui/Chat.kt`, memo `research/security/phase4-phone-vouch.md`): "I know them" signs a vouch under the thread's persona and sends it; "Show who knows me" sends the vouches received, one message; a reader counts only signers already among its own contacts and says "Pat and Sam know them" — never a score, never published, one hop only. **Walked on the live network 2026-09-14** with four desks (`mailbox vouched` / `voucher` ×2 / `knower`): Pat and Sam each claimed X's card and vouched; the reader claimed Pat and Sam as contacts and X as a stranger, held nothing about X ("known_by=[]"), asked, X showed its two vouches in one message, and the reader concluded "Pat and Sam — 2 of your contacts know this person" 18 s after asking. Two traps for the next run: a profile card is claimed once, so X cuts a fresh card after every claim; two Veilid nodes started in the same second failed one of them with "Could not initialize the protected store" — start them a few seconds apart. | "2 of your contacts know this person" on a three-phone walk |
