...
| A4 | Medium | ui/Hail.kt, Hailing.kt | A hail publishes name, ~1 km cell and a street-level destination unsealed | **fixed 2026-09-14** — the board carries the area a geocoded place sits in, never the name or the street; the precise destination is kept on the rider's phone and sent to the driver who claims, in the sealed thread. §16.17 already required this and the implementation did not follow it. **Wants a ride walk**: the flow is proven on two phones and this changes what the board says and adds one message on claim. |
