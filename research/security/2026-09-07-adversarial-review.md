...
| W15 | Low | core/src/state.rs | The 120 s contact window is not held by the state machine | **answered 2026-09-14** — the machine cannot hold it: the window closing leaves the state identical, and `transition` is pure by design (elapsed time arrives as an event, never from a clock). §18.4.1 gains rule 9 assigning it to the caller — refuse a late coda, destroy the session keys at the window's end — and the arm in core says so where somebody would otherwise assume it was covered |
| W16 | Low | §15.3.2 | "offer_commit is necessarily empty" has no wire meaning | **fixed 2026-09-14** — "necessarily empty" is all zeroes, refused otherwise on a payer-presented tap; core enforces it and §15.3.2 says it. **The conformance suite does not exercise it**: the second implementation does not model `TapPresent`'s fields, and no vector builds a payer-presented tap at all — a blind spot worth naming rather than papering over |
...
| D11 | Medium | app/src/, src-tauri/src/lib.rs | The desk has no second factor at all, so §15.5.1's spend gate cannot run on it | **open 2026-09-15** — the phone now gates every payment on a recent device unlock and asks for a PIN above a user-set threshold. The desk has neither: no device credential to ask about and no secret of its own, so anyone at an unlocked laptop can spend the desk's wallet through `pay_contact`. §15.5.1 now says in as many words that a client in that position MUST NOT treat the absence as satisfaction, which makes this a stated gap rather than a silent one. Closing it means giving the desk a secret — a passphrase-derived verifier like the phone's PIN — and that is its own piece of work. |
...
| A3 | Medium | res/xml/apduservice.xml, nfc/ | Standing card served over NFC from the lock screen; claims adopted automatically | **fixed 2026-09-14** — the tap service requires the device unlocked, so a pocketed phone no longer hands its standing card to a reader brought near it; and a card that is answered now says so, naming who became a contact and how to forget them |
| A4 | Medium | ui/Hail.kt, Hailing.kt | A hail publishes name, ~1 km cell and a street-level destination unsealed | **fixed 2026-09-14** — the board carries the area a geocoded place sits in, never the name or the street; the precise destination is kept on the rider's phone and sent to the driver who claims, in the sealed thread. §16.17 already required this and the implementation did not follow it. **Wants a ride walk**: the flow is proven on two phones and this changes what the board says and adds one message on claim. **Finished 2026-09-15**: the name was still published twice, on the card URI and in the card's plaintext inbox half, because a card's own half is written before there is anybody to seal it to. A hail's card now carries no name and an empty profile (§16.17), and the rider's name, picture, precise pickup and precise destination all travel after the claim — the name as §16.3.1's `INTRODUCTION` (kind 17), the places as ordinary sealed messages. Proven desk to desk: a nameless card claimed, the contact reading `Unnamed` until the introduction named it, then the pickup. Still wants the two-phone ride walk. |
