...
- Multi-round key exchange is *not*, in measurement, the fragile part: 2 rounds, 134 s, deterministic (§O1). Earlier drafts treated wallet-sync and key-exchange failure as the primary hazard; that was inherited caution rather than an observation. The protocol must treat multisig setup as a fallible sub-state-machine with explicit timeouts and fund-recovery paths, not a single call.
...
- **O1.** **Multisig — reframed by measurement, not resolved.** A 2-of-3 ceremony on Monero v0.18.5.1 converged in 2 exchange rounds and 134 seconds, deterministically, across three independent wallet services (`monero-spike/REPORT.md`). Round-trip fragility was **overstated**. The real problems are different and worse: Monero ships multisig **disabled by default**, with an upstream warning that funds may be unspendable or stealable (§8.2); **no RPC method enables it**, so a client must drive `monero-wallet-cli` out-of-band or bypass the RPC entirely; and a wallet can be **stranded halfway** — `prepare` and `make` succeed with the flag off, `exchange` refuses, and `prepare` then rejects the wallet as already multisig. The risk is upstream's experimental status and a missing API, not brittleness.
