...
**Draft 1.1.0-dev13 — Publications track (branch; 1.0.0-rc1 is the frozen line)**
...
### Changelog
- **1.1.0-dev13** — **Unlock the phone, open the app, tap (§15.5.1; backup keys 28–29).** §15.5.1's ladder could express the right policy and was set to the wrong one: it asked nothing below twenty dollars, which is a tier an attacker never has to clear, and asked for a secret above a hundred *once*, which is a tier a shoulder-surfer clears once and then spends through. The shipped shape is now the one a phone's own payment app already has. The floor is **zero** — every payment wants a device unlocked inside a two-minute window, and that window is now a policy field (`device_unlock_validity_s`) rather than an assumption, because the platforms answer a yes/no about a window and not an age. Above the one threshold the user sets, the secret is asked **every time** (`app_secret_every_time`). A device with no secure lock screen establishes nothing and falls back to that secret on every payment; a terminal with no credential of its own **MUST NOT** treat the absence as satisfaction. Both new fields ride a backup, optional on the way in and defaulting to the stricter reading, and a bundle written before they existed is opened by a frozen test rather than by assertion.
- **1.1.0-dev13** — **A hail's card carries no name (§16.17).** The rule §16.3.1 made possible, made normative for the one surface that needs it: a card pinned to a public board has its inbox half written before anybody has claimed, so anything that half carries is published to every reader of the board. A hail's card **MUST** carry no asserted name, in the URI or in the record, and **SHOULD** carry an empty profile. The rider's name, picture, precise pickup and precise destination travel afterwards, to the one driver who claimed, as an `INTRODUCTION`. §16.18's listing has the same exposure and the same remedy, and there it stays a **MAY**: a seller who posts week after week is a standing identity whose recognisability is part of what is being offered, and trading that away is the poster's call. Proven desk to desk: a nameless card claimed, then named by the introduction that followed it.
- **1.1.0-dev12** — **A card may publish less than it hands over (§16.3, §16.17; message kind 17).** A card on a public board is read by everyone who reads that board, so what it may carry is only what a stranger needs in order to *decide*. A rider hailing a ride should not have to publish their name to the neighbourhood to get one. The identity coda §16.3 has reserved since the beginning finally has a message kind: `INTRODUCTION` carries, in its payload, the same signed `CONTACT_ACCEPT` a card's inbox half carries, sealed in the thread instead of published on a record. Nothing new is invented and nothing is taken on faith — it opens under the persona it names and names the inbox the thread was born from, and a reader that cannot tie it to both refuses it. A hail therefore publishes an area, a cell, a fare and a persona, and the rider's name, their picture and the precise pickup travel once a driver has claimed; the driver's plate, car and photograph already travelled that way, in the sealed half of the claim. Four new vectors; 442 cases, both implementations agreeing.
- **1.1.0-dev12** — **"`offer_commit` is necessarily empty" gets a value (§15.3.2).** The field is thirty-two bytes and always present, so the rule a payer-presented tap was supposed to follow could not be written down, let alone checked. It is all zeroes, and a reader refuses a payer-presented tap carrying anything else.
- **1.1.0-dev12** — **A slash claim names who is claiming (§17.5; field 321).** `SLASH_CLAIM` carried the accept, the receipt, the transaction, the reason, the amount and a time, and nothing that said whose claim it was: an arbiter reads one out of a transport that may not say who sent it, and a payout goes to somebody. It carries `claimant` now, required, thirty-two bytes, and all zeroes is refused — so a claim lifted out of one thread and filed in another still says whose it is.
- **1.1.0-dev12** — **The claimant's half of a contact inbox is sealed, not merely signed (§16.9).** A card on a board publishes its inbox's record key to everyone who reads that board, and until now the reply was plaintext under a signature — so every reader learned the claimant's persona, asserted name, reach-me identifiers, and, on a hail, the driver's plate and the photograph of their car. The claimant now seals its signed half to the prekey bundle the issuer published in subkey 0, in §16.11's HPKE envelope under its own `info` string, with the inbox record key as associated data so a reply cannot be replayed into another card's record. An issuer MUST NOT accept an unsealed reply. No new fields and no new object: the envelope is `SEALED_MESSAGE` and the half inside it is the same signed `CONTACT_ACCEPT` as before. Proven desk to desk on the live network.
- **1.1.0-dev12** — **Vouching gets its object (§9.2; fields 317–319; object type 29).** `VOUCH` is the smallest signed thing in the protocol — signer, subject, a time, sealed under the signer: *I know this persona*, said by someone who met them. It travels as a receipt does and is weighed the same way, by the reader alone: a vouch counts only when its signer is already among the reader's own contacts, the answer is worn in words ("2 of your contacts know this person"), nothing is published, and no distance beyond one hop is computed. Refused at the shape: a vouch from nowhen, a persona vouching for itself, a signature under any key but the one named inside. And §9.5's seller's minimum gets its field: `RENTAL_NOTICE` may carry `min_burn` (320), the least a buyer's persona must have burned for the poster to deal, which a buyer's client MUST show before the buyer commits. Six new vectors.
- **1.1.0-dev11** — **Costly identity gets its objects (§9.5, §9.2; fields 304–316; object type 28).** `BURN_PROOF` carries a persona's proof that it sacrificed XMR — the transaction, the amount the out-proof proves, the block, Monero's `OutProofV2` whose message names the persona and a purpose — as a §18.3 envelope under that persona; `ATTESTATION`, registered as type 12 since 0.47 and never given a shape, is now §9.2's rated receipt: signer, subject, the settled amount, a rating from 1 to 5, a time, the transaction and one sentence, signed under the signer. Both are shapes a reader refuses before asking a node anything: a burn of nothing, a burn without a block, an in-proof where an out-proof is required, a torn proof, a rating outside the set, an attestation from nowhen, a persona attesting to itself, a signature under any key but the one named inside. What the reader then does with the chain — verify the out-proof against the burn address for the proven amount, ask its own node and the second opinion whether the transaction is in a block, weigh a receipt by its signer's burn — is §9.5's and lives with the wallet, not here. Nineteen new vectors; 430 cases, both implementations agreeing.
- **1.1.0-dev10** — **The reply to a contact card is signed (§16.9, fields 302–303), and the security pass that found it.** The card was always signed; the two halves of its inbox were bare maps, and the persona in the claimant's reply was taken on faith by every reader that keys trust by persona — so whoever could claim a card (anyone, for a card on a board) could write a reply naming the issuer's arbiter or a friend and have that contact rebound to the writer's outbox and prekeys, with the ceremony rounds and bills that follow sealed to the wrong keys. Both halves are now `CONTACT_ACCEPT` version 2 carried as a §18.3 envelope under the persona named inside, and the body names the inbox it was written into (302) and which half it is (303), so a signed half cannot be lifted between inboxes or subkeys; version 1 is refused outright, and a claimant refuses a subkey 0 whose persona is not the card's. Five vectors pin the envelope: a body signed by another key, a bare map, version 1, a role outside {0, 1}, an empty inbox. 411 cases, both implementations agreeing. Also from the same pass, recorded here because they change what a client MUST do rather than the wire: a reader of a group roster (§16.24) MUST refuse a generation whose member list is not a superset of the one it holds, whose owner is not the sender, or whose number is not the successor of the one it holds; a `feed.json` whose `persona` is not the home it was fetched from MUST be refused (§16.23); a fetch of any bundle MUST carry a byte ceiling and refuse an index that declares more; the Monero node's fee estimate is a claim and a client MUST NOT sign a transaction whose fee exceeds a locally computed ceiling; a second opinion that cannot be reached is *unknown*, never *yes*, and goods MUST NOT leave on a pool sighting without an operator's explicit, capped choice (§15.11); a co-signer of an escrow release MUST size its consent from the transaction's inputs, outputs and fee, never from a scanned balance.
- **1.1.0-dev9** — **A group keeps its words on one shared record (§16.24, fields 290–299); a persona has a home and a feed (§16.23); a listing carries a bundle behind its notice (§16.18.3); and a picture where a person decides — the cover (300) and the car (301).** §16.19 fanned every group message into N−1 sealed writes and made a lap N−1 reads, and sending needed a full mesh. A **group board** is one DHT record with the SMPL schema per generation: every member writes only their own ring of pages, each write signed by their own key, so a message is one write, a lap is one inspection, a newcomer reads the recent pages instead of nothing, and a sender can no longer say different things to different members. The **first member entry is a nameplate** — the SHA-256 of `"ducat group board"`, the group id and the generation — that nobody holds a key for, so two groups of the same people are two records and one group's generations are two records even when the list did not change. `PAGES` is 4, a board holds at most 255 members, and a page is bounded by the lesser of 32 KiB and 1 MiB ÷ subkeys; pages are sealed under the generation's key with the record key and subkey as associated data, §16.20's way, so a page cannot be moved and still open. Readers merge by `(sender, GB_SEQ)` and order by `GB_TS` then sender key, so two phones show one order. Growing the group forms the next generation — a fresh record, a fresh key, and the `GROUP_ROSTER` payload gains the generation, the owner, the key and `PAGES` as keys 3–6; a roster without them describes a §16.19 group without a board, which keeps working. Nobody is removed, and leaving is local. **What the board trades away is written down**: forward secrecy becomes per-generation and deniability goes, because the record signs each page with the persona key a card already binds; consistency and history are gained; money stays pairwise. A **home** (§16.23) is a §16.22 site whose record owner is the persona key itself, so every holder of the card can compute the address and only the persona can write it; its bundle MAY carry `feed.json`, a closed set of at most 200 posts in a text subset that cannot walk a reader out of the room, refused whole on anything else. To heart a persona is to keep its home; the timeline is the hearted feeds merged newest first, and there is no global timeline and no follower count anywhere. The author serves the edition it published — a client MUST NOT replace its own home with one fetched back from the network. The share a listing names (§16.18.3) is now a **bundle**: `listing.json` in the manner of `feed.json` — a description of at most 8 000 characters, at most 24 pictures, 8 files and 32 spec pairs, and the price as the seller typed it — fetched only when the listing is opened, refused whole when it does not open with the pictures shown alone, and never able to change the price, the area or the card, which stay on the signed notice. Language boards narrow and the bare board is everyone: a notice posted with a language is posted to both, two stamps paid honestly. A publication notice MAY carry a **cover** (300) under the listing thumbnail's rules, and a driving profile MAY carry the **car's picture** (301), which rides the backup's roster entry beside the plate as the avatar rides beside the name. The registry's unallocated row now begins at 302; **299 is never to be assigned**, being the group page's strict-reader probe. A new vector kind, `group.page`, pins that entries ascend, references travel in pairs, reactions and retractions name a target, and the probe is refused. 406 cases, both implementations agreeing.
- **1.1.0-dev8** — **A listing gets pictures, split by who pays for them (§16.18.3, fields 287–289).** A marketplace of words is a marketplace nobody browses, but a board slot is written by one person and read by everyone, and the reader is a phone that is merely looking. So: **one small picture inline** (287, ≤10 KiB, PNG/JPEG/WebP, the avatar's three formats checked the avatar's way) arrives with the read the browser was already doing — no second network operation, no peer discovery, and it still shows when the seller's device is off — and **the full-size gallery rides a swarm share** (288–289, share key + index digest, ~160 bytes, both halves or neither) fetched only by whoever opens the listing. The cap is sized against the *sweep* rather than the picture: 18 boards × 8 slots is 144 notices, so 10 KiB apiece is ~1.4 MB a lap against 45 kB of words today, and bigger is not a better picture but a slower marketplace for everybody reading it. Refused rather than trimmed at both edges, because a slot is signed over its bytes and a reader that quietly accepted an oversized one would be verifying something no other reader would; empty is `MALFORMED` too, omission being the one spelling of "no picture". **A client MUST NOT fetch galleries while browsing** — peer discovery is tens of seconds before a first piece moves, and a browse screen starting eight of them has turned a board read into a stall. Two consequences a client must say out loud: a swarm serves only while somebody seeds it, so a seller who is offline has a gallery that does not load (the thumbnail still does, which is why it is on the board); and fetching a gallery is a peer connection to the seller's node, so **opening a listing's pictures tells them somebody is looking** where reading the board tells them nothing — browsing is unobserved (§5.2.3), and a client SHOULD NOT prefetch on the reader's behalf. Every published picture MUST be re-encoded rather than passed through: a photograph carries where it was taken, and a listing is a public board. Seven vectors pin the cap at 10240 and one past it, the format check, the empty spelling, and both halves of the gallery pair. 386 cases, both implementations agreeing.
- **1.1.0-dev7** — **A reader asks to be sold (§16.20, kind 16, field 286).** The publication rail had a handover and no way to request one: a reader could be *given* a period but never *ask* for one, so every sale began out of band. `PUBLICATION_WANTED` carries exactly one thing — the period id, a label ≤64 — and the ask is deliberately not the mirror of the handover: naming what you *want* is field 286, naming what you *give* is 259, and they are separate numbers because a reader who could write the giving field would be describing a capability nobody granted. No amount rides it either; the reader does not price the goods. The publisher answers on one of two paths, and **free is a path, not a zero** — an unpriced notice gets its key straight back with no bill and no settlement, while a priced one raises an ordinary §16.13 bill against that one reader for that one period and the existing reconcile loop delivers the key when it settles. Routing the money through §16.13 rather than inventing a payment path is what puts the purchase on the activity statement with a line item naming what it bought, because a purchase nobody can account for later is one they will not make twice. An ask is a request and never a claim: ignoring it is allowed, refusing it is allowed, asking twice is not an error, and the publisher's own record of what was sold — not the wire — is what stops a delivered period being billed a second time. **The number is 286 and not 265**, which is where it was first written: 265–275 are `PUB_NOTICE`'s body and stamp, and the collision passed every gate — 386 vectors agreeing, both implementations — because no vector puts a notice and a sealed message in one object, so nothing ever compared them. §18.4.2's registry is one namespace and not one per object; the audit that catches this reads the whole registry, and the reason it stayed quiet was that its output was read from the tail rather than the top. 276–277 and 285 stay pinned probes; the table's stale `276+ Unallocated` row, which had been wrong since the attachment's second road shipped, now ends at 287. Four new vectors pin the ask, its empty form, the field on a text, and an amount aboard it. 379 cases, both implementations agreeing.
- **1.1.0-dev6** — **A publication is discovered like a kayak (§16.18.2, `PUB_NOTICE`, fields 265–275).** The digital good takes the same boards: a claim-once publish card, a title, an optional blurb and an optional per-period price ride a notice whose stamp block is §16.18.1 verbatim in its own namespace — and whose board name, inside the signature like a slot, carries the where: `topic:<category>[.<lang>]` worldwide (six pinned categories, `other` the valve), `local:<cell>` for the town paper, cross-posting two honest stamps. Absent price means free and an explicit zero is `MALFORMED` — one meaning, one encoding. Claiming the card IS subscribing; everything after is §16.20 unchanged. Eleven new vectors pin both edges including the family's sealed form on a topic board and its unknown-field edge (276) — which caught the second implementation answering `MALFORMED` where core answers `UNKNOWN_FIELD` on every strict reader's leftover path, a divergence no vector had ever exercised; the checker now agrees with core in core's own words. **And the backup dresses every hat (§4.3):** each roster entry now carries its persona's own §16.9 profile — public name, avatar, reach-me identifiers, pronouns, the car, and the share switch (written only when off; absence decodes as on, the stored default) — as optional per-persona keys 4–13, because the presentation follows the hat and a restore that returns three keys wearing one face has quietly merged what the compartments kept apart. The primary's copy still also rides the legacy top-level fields, so a reader from the single-profile era restores what it always did; per-persona wins where both exist. One vector pins the enumeration at its edge, both sides. **And the attachment grows a second road (§16.15, fields 278–279):** the same sealed blob — key, nonce, length, hash and mime unchanged, all-or-nothing as ever — may ride a swarm share instead of a DHT record, which is how a file outgrows the 1 MiB record without outgrowing the thread: share key and digest travel together inside the sealed message, exactly one transport is present (a record or the swarm, never both), and the big road carries its own bound (256 MiB) past which distribution is the publication machinery's job. 276–277 are reserved for ever: 276 is the strict-reader probe a vector already pins as unknown, and assigning past a probe is the bond_proof collision (0.60) relearned. Four vectors pin the two-road rule at its edges. **And pages that travel like publications (§16.22, `SITE_HEAD`, fields 280–284):** a site is a multi-file bundle on the swarm plus one mutable head at a stable record key — version, title, the current bundle's share and digest, updated-at — rewritten in place to update the site, mirrored by readers who choose to, rendered in a sealed room: the viewer serves every request from the fetched bundle and nothing else, scripts off, `ducat:` links the only door out, so nothing on a page can see its reader. Strict reader, closed set, unknown-field edge pinned at 285. Four vectors. 375 cases, both implementations agreeing.
...
**The roster, each hat dressed.** A bundle also carries every persona the device wears (key 27), primary first, and each roster entry carries that persona's own §16.9 profile as optional per-persona keys 4–14 — public name, avatar, reach-me identifiers, pronouns, the car's model, colour and plate, the share switch (written only when off), and **the car's picture at key 14**, which rides beside the plate exactly as the avatar rides beside the name: the same bytes field 301 carries on a driving profile, so a driver restored from a bundle is the same car at the curb and not a description of one. None of the profile is re-validated on the way in — it was checked when it was entered and is checked again when it is published, and a bundle that will not open over a picture's magic number is a wallet held hostage to a formatting rule; the trust-boundary rule below is for fields whose *construction* the restore itself acts on.
...
**Vouching (wire object `VOUCH`, type 29, fields 317–319).** A persona that has met another in person MAY sign a `VOUCH`: signer, subject, a time — and nothing else, no rating, no amount, no sentence, because a vouch says one thing, *I know this persona*, and the less it carries the less it leaks. It is sealed under its signer (§18.3) as a receipt is, refused at the shape when it names nowhen or vouches for its own signer, and travels the same road: the signer sends it to its subject as a `ducat:vouch/<hex>` link, kept only when sent by its signer and about one of the reader's own personas; a persona shows what it holds as `ducat:vouches/<hex>.<hex>…`, newest first, as many as fit one message. What a reader does with it is arithmetic over its *own* contacts: a vouch counts only when its signer is a contact the reader already holds, and the answer is worn in words — "2 of your contacts know this person" — never as a score, never published, never forwarded. Distance beyond one hop is not computed: a friend of a friend is a stranger with a story.
...
### 9.5 Costly identity on Monero — proof of burn
...
**Verification, by the reader.** A client shown a burn proof MUST: (1) check that the message names the persona presenting it, refusing otherwise; (2) ask its own Monero node for the transaction and verify the `OutProofV2` against the burn address, accepting only the amount the proof proves — never the amount claimed beside it; (3) ask the second-opinion node whether that transaction is in a block, and treat *unknown*, *in the pool* and *unreachable* as **not yet**, never as *yes* (§17.5's three answers, the rule the money path already follows); (4) refuse an amount under the floor. A proof is checked once per persona and the verdict kept with the contact; it never expires, because a burn cannot be undone.

**The floor.** A burn under `BURN_FLOOR_PXMR` counts for nothing. The constant starts at 0.01 XMR — about the price of a coffee — and is a client policy, not a wire rule, so it can move without a draft. It exists so that the smallest useful burn is still a cost; the gate below, not the floor, is what protects money.

**What a client shows and does.** A burn is rendered in words beside the persona wherever a decision is made — a listing, a hail, a till, a thread: "burned 0.05 XMR, since March", never a number out of context. A client SHOULD score a persona from its largest single burn rather than the sum of many (JoinMarket's reason: many small identities must not add up to one large one), from the age of its first burn's block (a birth certificate nobody can backdate), and from §9.2's rated receipts weighted by each signer's own burn. **The gate is normative:** a client MUST warn, and MAY refuse, when the amount a person is about to put at risk with a counterparty exceeds what that counterparty's burn and bond together cover; a seller MAY publish a minimum for buyers (`RENTAL_NOTICE` field 320), and a buyer's client MUST show that minimum before the buyer commits to anything.

**How a proof travels.** Until a message kind carries it, a persona shows its burn by sending the `BURN_PROOF` envelope as a `ducat:burn/<hex>` link in a sealed message (§16.10); a client recognises the prefix, draws it as a proof rather than as text, and offers to check it. What the check finds is kept beside the contact and worn as words — "burned 0.05 XMR, since block N" — nowhere else.

**How a receipt travels.** §9.2's `ATTESTATION` goes the same road. The signer sends it to its subject as a `ducat:attest/<hex>` link; the subject's client keeps it only if the envelope opens under the persona that sent it — a receipt handed on by anyone but its signer is discarded — and only if it is about one of the reader's own personas. When somebody asks for the record, the subject sends what it holds as `ducat:record/<hex>.<hex>…`, envelopes joined by dots, newest first, as many as fit one message (§16.10's 2000 characters — about three) and sixty-four at most; the reader keeps each one that opens and is about the sender, and drops the rest without comment. The signer of a receipt, and the persona showing a record, is the persona that owns the thread — the one the message is sealed under — so that signer and sender can agree. What the reader then says about the record is its own arithmetic: how many receipts it read, and — the number that means anything — how many distinct signers among them have a burn the reader verified itself, one voice per signer however many receipts that signer wrote, rated by the latest. A record is therefore never a score somebody else computed: it is envelopes, and every reader weighs them again.

**Privacy accounting (§16.6's habit).** The burn transaction is an ordinary Monero transaction: its existence is public, its amount, sender and the persona are not. The proof reveals the amount to the reader and binds the persona to it — to that reader, in that thread. A persona SHOULD burn from its own wallet, so that two personas of one person are not tied by the funds they burned. Nothing here is written to a board, a ledger, or a record anyone but the persona controls; the proof lives in the persona's attestation record (§9.2) and rides the backup with it.

**What this does not do.** It does not prove a person is honest, unique, or safe (§9.4); it makes an identity cost something to obtain and fraud cost something per identity. A scammer who expects to net more than the burn will burn. The score is a floor under which one should not trade unbonded, and the gate says so in the client's own words.

**Attestations, restated for §9.2.** An `ATTESTATION` (object type 12) is a rated `RECEIPT`: the subject persona, the signer's persona, the settled amount, a rating from a closed set, and the receipt or transaction it stands on, signed by the signer under §18.3. It is issued after a settled deal into the subject's attestation record, whose writer key the subject holds (§4.3 already carries those keys in the backup). A reader weights each attestation by the signer's own verified burn or bond, so a review from an unburned persona is a sentence and not a score. Fields 310–316 in §18.4.2, object type 12; vectors pin the shape, the closed rating set, and that a persona cannot attest to itself.
...
- **O4. Narrowed, and the trade is now paid in money rather than in linkability** (§9.5, §9.2). The problem was never that reputation is impossible — it is that the usual construction buys it by publishing a history, which is the thing §4 refuses. Three pieces changed the shape. A **proof of burn** makes an identity cost real XMR, which is a sybil price that needs no history at all; a **rated receipt** is an envelope handed to one reader in a sealed thread rather than a score anybody publishes; and a **vouch** is counted only when its signer is already one of the reader's own contacts, one hop, so no graph is walked and none is disclosed. Every verdict is the reader's own arithmetic over what it was handed. **What is not closed:** a persona that transacts repeatedly is still linkable *to its counterparties* by construction — the receipts name their signers to the reader — so the trade is moved, not abolished. §9.5's private form (blind or linkable-ring receipt tokens) is the answer to that half and is not built. And burning prices an identity without proving anything about conduct: somebody willing to spend can buy a plausible history, which is a speed bump with a stated price rather than a wall.
...
- **`amount_authority` MUST be `open`.** A customer's phone does not know the price of a coffee, and a payer-presented tap has no offer to commit to, because the offer does not exist until the till makes one. `offer_commit` is thirty-two bytes and always present, so "no commitment" needs a value rather than an absence: it is **all zeroes**, and a reader MUST refuse a payer-presented tap carrying anything else. (Through 1.1.0-dev11 this said the field was "necessarily empty", which the wire could not express.)
...
2. **Both kinds of evidence expire.** Without a validity window, "deliberate" decays into "happened at some point today", and "unlocked" decays into "has a lock screen".
...
#### The shape a client SHOULD ship

The ladder above is what the policy can express. What a client should *set it to* is narrower, and it is the shape a phone's own payment app already has, because that is the one people already know:

- **`device_unlock_at` is zero.** Every payment wants a device unlocked within `device_unlock_validity_s` — two minutes. Unlock the phone, open the app, tap. There is deliberately **no `None` tier in the shipped defaults**: what leaves is money in a wallet nobody can reverse, not a transit fare, and a tier that asks nothing is one an attacker never has to clear.
- **`app_secret_at` is the one number the user sets**, defaulting to 100 of whatever they price in.
- **`app_secret_every_time` is on.** A payment at or above that threshold gets its own entry rather than resting on one from a minute ago. This is the difference between "a PIN above a hundred" and "a PIN above a hundred, once": somebody who reads a PIN over a shoulder and then takes the phone gets one large payment out of it rather than as many as they can tap inside the window.

`device_unlocked` is a **yes/no about a window, not an age**, because that is what the platforms can answer: Android's keystore will hold a key bound to a recent authentication and refuse to use it once the window has lapsed. So the window lives in the policy and the client asks the platform about exactly that number. A device with **no secure lock screen at all** answers "no" — nothing has been established — and the app's own secret is what is left, which means such a device asks for the secret on every payment. That is the correct direction and not a dead end.

A **desk or terminal with no device credential** is the same case: it cannot satisfy `DeviceUnlocked` at all, so either it has a secret of its own or it has no verification. A client in that position **MUST NOT** silently treat the absence as satisfaction.
...
4. **The driver completes; the rider releases.** The payee proposes the FROST release to their own wallet; the funder's co-signature is a **screen, never an automatic signature** — §15.5's confirm rule surviving into escrow. A ride ceremony's release proposal parks until the human says yes; only a plain deposit-return (§17.9's 2-of-2 bond) may auto-co-sign. Consent is sized from the transaction itself (§17.9) — inputs, outputs and fee, read back out of the bytes being signed, never from a scanned balance — and this leg would survive without it anyway: the sweep can only pay where the proposer says, and the proposer is the party the fare was always for, so a redirected destination changes who holds the driver's key, not who earned the fare.
...
### 16.3.1 The coda as a message: `INTRODUCTION` (kind 17)

The flow above is the tap's, where two people are standing together and the transaction is already over. The same act is needed in a second place, and for a different reason: **a card on a public board is read by everyone who reads that board.** §16.9 lets an issuer publish a card and §16.17 puts one on a kerb, and everything the card's own inbox half carries — the issuer's asserted name, their picture, their pronouns — is published with it, because that half is written before anybody has claimed and there is nobody to seal it to.

So a card MAY publish less than it hands over. What a board carries is only what a stranger needs in order to **decide**: for a hail, the area, the two cells, the fare, and the persona whose burn and receipts a reader can weigh (§9.5). A rider does not have to publish their name to the neighbourhood in order to get a ride.

The rest travels afterwards, as an ordinary sealed message. `INTRODUCTION`, message kind 17, carries in its `payload` the same signed `CONTACT_ACCEPT` a card's inbox half carries — the identical object, sealed in the thread rather than published on a record. Nothing new is invented and nothing is taken on faith:

- It **opens under the persona it names** (§18.3), so an introduction is somebody's own account of themselves and not a third party's account of them.
- It **names the inbox the thread was born from** (field 302), so one cannot be lifted out of one relationship and replayed into another.
- A reader that cannot tie it to both **MUST** refuse it, and a reader **SHOULD** show a contact that renames itself as having renamed itself, rather than silently adopting the new name. The first name you were given is the one you decided on.

A hail is the worked example. The board carries no name. The driver claims, and their half of the inbox — sealed since draft dev12 — carries their name, their plate, their car and its colour and its photograph, because §15.12 has always said a plate travels only on a driving claim. The rider answers with an `INTRODUCTION` carrying their own name and picture, and with the precise pickup and destination that the board deliberately did not have. Both sides end up knowing what they need to meet, and the board learned neither.
...
**Both halves of the inbox are signed.** What the issuer writes into subkey 0 and what the claimant writes into subkey 1 is the same object, `CONTACT_ACCEPT` (§16.12), and each is carried as a §18.3 signed envelope under the persona key named inside it. A reader learns which key to check from the body and then checks it; a half that verifies under any other key is somebody claiming a persona that is not theirs, and is refused. The body also names the inbox record it was written into (field 302) and which half it is (field 303, `0` issuer, `1` claimant), so a signed reply cannot be copied from one card's inbox into another's, nor an issuer's half be passed off as a claimant's. This is the identity binding cards had been missing: the card itself was always signed, but the reply to it was a bare map, and every place a client keys trust by persona — the arbiter it configured, the friend it already holds, a group's roster, a heart — took the persona in that map on faith. Anyone who could claim a card could therefore write a reply naming *someone else's* persona and have the issuer rebind that contact to the writer's outbox and prekeys. Version 1 of the object, the unsigned map, MUST be refused; there is no older-but-fine reading of it. The claimant MUST also refuse a subkey 0 whose persona is not the persona the signed card names. The writer key in the card proves possession of the slot and nothing more.

**And the claimant's half is sealed, not merely signed.** The issuer's half goes into subkey 0 before anybody has claimed the card, so it can only be signed; the claimant's half can do better, and on a public surface it must. A hail or a listing publishes the card URI, so every reader of that board holds the inbox's record key — and a signed reply is a reply all of them can read: the claimant's persona, the name they asserted, the reach-me identifiers the purpose admitted, a driver's plate and the photograph of their car. So the claimant **MUST** seal its signed half to the prekey bundle the issuer published in subkey 0 (§16.11's `SEALED_MESSAGE` envelope, HPKE, `info` = `"DUCAT-v1"‖0‖"CONTACT_ACCEPT"‖0‖suite` so a reply can never open as a thread message or the reverse), with the **inbox record key as the associated data** — a sealed half lifted into another card's record does not open, so nobody can replay somebody's reply into a handshake they never joined. The issuer opens it with the prekey the envelope names and retires a one-time key by using it; a claimant whose issuer has no one-time key left falls back to the signed prekey, and its client SHOULD say so rather than treat the two outcomes as one. An issuer **MUST NOT** accept an unsealed reply: a downgrade path here is the hole itself. What a board reader now finds in subkey 1 is noise, and what the claimant still reveals by claiming at all is that somebody claimed.
...
**A sealed message is at most 16 KiB on the wire.** That is the HPKE ciphertext ceiling a reader enforces before it touches a key: room for the largest legal body below at UTF-8's worst case, for a `GROUP_ROSTER` naming 255 members (§16.19), and for the framing and the tag — and half a DHT subkey, so one message never needs two.
...
```
PREKEY_BUNDLE { signed_prekey, one_time[ (id, key) … ], expiry }   inside the persona-signed CONTACT_ACCEPT (§16.9)
        │
        ▼
...
CONTACT INBOX     SMPL(1, [writer])      created by the card issuer
   subkey 0   →   issuer's details      (owner writes; signed by the issuer's persona)
   subkey 1   →   claimant's reply      (card holder writes; signed by the claimant's persona)
...
```
HAIL_NOTICE {
  version      (203)  uint, = 2 — a version-1 notice carried no author and
                      no proof of work, and is refused rather than read
...
**The card carries no name.** A hail's card is pinned to a board every phone in the neighbourhood reads, and a card's own inbox half (§16.9, subkey 0) is written before anybody has claimed it — there is nobody to seal it to, so everything it carries is published. A rider should not have to publish their name, their picture and their pronouns to the neighbourhood in order to get a ride. So a **hail's** card **MUST** carry no asserted name, in the URI or in its inbox half, and **SHOULD** carry an empty profile: the notice publishes an area, two cells, a fare and a persona, which is what a driver needs to decide. The rest travels afterwards, as §16.3.1's `INTRODUCTION` — the rider's name and picture, the precise pickup, and the precise destination — to the one driver who claimed. The driver's own name, plate, car and photograph came the other way at the same moment, in the sealed half of the claim (§16.9, W4), because §15.12 has always said a plate travels on a driving claim and nowhere else.

§16.18's listing sits on the same kind of board and has the same exposure, and the same remedy is available to it — but the rule there is a **MAY**, not a MUST, and deliberately: a hail is one stranger asking for one ride, while a seller who posts week after week is a standing identity whose recognisability is part of what is being offered. Who that trades away is the poster's call, not the format's.
...
RENTAL_NOTICE {
  ...
  deposit      (227)  uint, what *each* side stakes (§15.12); 0 is legal,
                      and always written — an absent field is MALFORMED,
                      so one listing has one encoding
  ...
  // the pictures (§16.18.3)
  thumb        (287)  bytes, ≤ 10 KiB, PNG/JPEG/WebP — one small picture,
                      inline. MALFORMED empty, oversized, or another format
  gallery      (288)  text, ≤ 128 — a swarm share carrying the full-size
                      pictures
  gallery_dig  (289)  bytes, 32 — its index digest. With 288 or not at all

  // what the poster asks of a buyer's name (§9.5)
  min_burn     (320)  uint, piconero — the least a buyer's persona must have
                      burned for the poster to deal. Absent means nothing is
                      asked; MALFORMED written as 0. A buyer's client MUST
                      show it before the buyer commits to anything
}
```
...
**What is on the board is what a stranger needs to *decide*. What they need to *arrive* is not.** The address, the plate, the door code, the photographs of someone's living room — none of it appears here. Those pass through the sealed thread after the two of them have agreed, because a listing is an advertisement, and an advertisement everyone can read must not double as a burglary brief. A client MUST NOT put an exact location or a registration plate on a board.

**Photographs are the one exception, and it is a narrow one.** A marketplace of words is a marketplace nobody browses, so a notice MAY carry the single bounded `thumb` (287) §16.18.3 defines and a publication's `cover` (300) under the same rules — one small picture, re-encoded rather than passed through, because a photograph carries where it was taken and a board is public. Everything else stays off: the gallery rides a swarm share that only a reader who opens the listing fetches, and the rooms, the keys and the street stay in the thread. The rule this section is really making is not "no pictures" but **nothing that helps a stranger arrive**, and a 10 KiB thumbnail of a record player does not.
...
A marketplace of words is a marketplace nobody browses. But a board slot
is read by everyone and written by one person, and the reader is a phone
that is merely looking — so the pictures are split by who pays for them.

**The thumbnail (287) rides the notice.** One small picture, at most
10 KiB, PNG, JPEG or WebP — the same three formats an avatar may be, and
checked the same way, because a decoder handed bytes whose format it must
guess is how a picture becomes an exploit. It arrives with the read the
browser was already doing: no second network operation, no peer
discovery, and it still shows when the seller's phone is off. The cap is
sized against the *sweep*, not the picture: eighteen boards of eight
slots is 144 notices, and at 10 KiB that lap carries about 1.4 MB of
images where today it carries 45 kB of words. Bigger is not a better
picture; it is a slower marketplace for everyone reading it.

Refused rather than trimmed, at both edges, because a slot is signed over
its bytes: a reader that quietly accepted an oversized thumbnail would be
verifying something no other reader would, which is §18.1's divergence in
picture form. Empty is `MALFORMED` too — omitting the key is how a
listing says it has none, and two spellings of nothing is what §18.1
exists to refuse.

**The gallery (288–289) does not touch the board.** Full-size pictures
ride a swarm share, exactly as an attachment's second road (§16.15) and a
site's bundle (§16.22) do, and the notice carries only the share key and
the index digest — about 160 bytes. Both halves or neither: a share key
...
browse screen that starts eight of them has turned a board read into a
stall — which is the failure this split exists to avoid.

Two consequences worth stating plainly, because a client must tell the
person about both. A swarm serves only while somebody seeds it, so **a
seller whose device is off has a gallery that does not load**; the
thumbnail still does, which is why it is on the board. And **fetching a
gallery tells the seller somebody is looking** — it is a peer connection
to their node, where reading the board tells them nothing. Browsing is
unobserved (§5.2.3); opening a listing's pictures is not, and a client
SHOULD NOT prefetch them on the reader's behalf.

Every picture a client publishes MUST be re-encoded rather than passed
through — §16.9's rule for avatars, for the same reason and more of it:
a photograph carries where it was taken, and a listing is a public board.
...
`GROUP_ROSTER` carries the member list in its payload (canonical CBOR; bounded like a ceremony round's). The creator's first roster **is** the invitation. Any member may add anyone: adding sends the grown set to everyone including the newcomer. **Nobody can be removed.** That is not a policy shortfall — removal is the one roster operation that needs a consensus a peer-to-peer group cannot have, and nothing can un-tell a person a group id. A grow-only set needs no consensus at all: merging two views is a union, unions commute, and every member's roster converges to the same set whatever order the adds arrive in. **Leaving is local.** A member who wants out stops reading, writing and showing the group; nothing is sent, because there is nothing to send — the roster cannot shrink, and the others keep talking. A later roster that moves the group to a newer board generation (§16.24) is an invitation again, and a client SHOULD treat it as one.
...
**The ask (kind 16, `PUBLICATION_WANTED`, field 286).** A reader who wants
a period says so on the thread. The message carries one field — field 286,
the period id, text ≤64 — and that asymmetry is the whole design: the ask
names what is *wanted*, the handover names what is *given*, and they are
separate numbers because a reader who could write field 259 would be
describing a capability nobody granted them. A kind-16 with no period is
`MALFORMED`; field 286 on any other kind is `MALFORMED`; and no amount
rides it, because the reader does not price the goods — naming a number
here would be an offer, and the publisher's price is the publisher's.

What happens next is the publisher's, and there are exactly two answers:

- **Free** — the notice carried no price (§16.18.2: absent means free, and
  an explicit zero is `MALFORMED`, one meaning one encoding). The
  publisher sends the `PUBLICATION_KEY` straight back. No bill, no
  settlement, no wait; free is not a zero-priced sale, it is a different
  path, and a rail that could only sell would make *free* mean *broken*.
- **Priced** — the publisher raises an ordinary §16.13 bill against that
  one reader for that one period. The reader pays it like any other bill,
  and the reconcile loop that already delivers keys on settlement delivers
  this one. Nothing new carries the money.

The bill is the point of routing a purchase through §16.13 rather than
inventing a payment path here: it lands on the activity statement with a
line item saying what it was for, so a reader scrolling their own spending
sees *"2600, issue-12"* and not an unexplained outgoing amount. A purchase
nobody can later account for is a purchase they will not make twice.

An ask is a request and never a claim. A publisher MAY ignore it, MAY
answer it later, and MAY refuse — there is no obligation on the wire and
no timeout; a reader who receives nothing has learned only that. Asking
twice is not an error either, because the publisher holds the only record
of what was sold, and a duplicate ask for a period already delivered is
answered from that record rather than billed again.
...
A site is a directory — `index.html` and whatever it references — carried
as one multi-file swarm share (§16.20's engine), named by one mutable
**head** at a stable DHT record key. `ducat:site/<record-key>` is the
site's address for the life of the site; the head (subkey 0) is whatever
that address currently points at.

**`SITE_HEAD`, fields 280–284.** `version` (=1), `title` (≤80 chars),
`share` (the current bundle's swarm share key, ≤128 chars), `digest`
(the bundle's index digest, 32 bytes), `updated` (epoch seconds,
advisory). Strict reader, closed set: an unknown field is
`UNKNOWN_FIELD`, and the edge is pinned at 285 on both implementations.
Updating a site is re-seeding a new bundle and rewriting this head in
place — the record's owner keypair is the write authority, exactly as it
is for the shelf, and a reader who saved the address keeps it across
every update.

**The sealed room, normatively.** A conforming viewer MUST resolve every
resource request from the fetched bundle and nothing else — no network
of any kind from rendered content — MUST NOT execute scripts unless a
future revision says otherwise, and MUST hand `ducat:` URIs to the
application's own flows; they are the page's only working exits. The
reasons are §2's, concentrated: one external fetch hands the reader's
address and timing to a third party, per-visitor beacon URLs make it
targeted, an unfetched resource is unsigned content inside a
digest-verified page, and a bundle with clearnet dependencies neither
works offline nor survives its publisher. A publisher tool SHOULD refuse
to seed a bundle that references external resources, so the page fails
at the keyboard rather than on a stranger's phone.

**Survivability and choice.** Readers MAY re-serve a fetched bundle
(§16.20's mirror rule); a viewer SHOULD make that a visible per-site
choice rather than a default, because hosting somebody's storefront is a
...
| 276–277 | never to be assigned — strict-reader probe values pinned by vectors | **Reserved** |
| 278–279 | attachment swarm transport (§16.15) | **Assigned** |
| 280–284 | site head (§16.22) | **Assigned** |
| 285 | never to be assigned — the site head's strict-reader probe, pinned by a vector | **Reserved** |
...
| 302 | `DET_INBOX` — the inbox record a signed half of a contact inbox was written into (§16.9) | **Assigned** |
| 303 | `DET_ROLE` — which half: `0` the issuer's (subkey 0), `1` the claimant's (subkey 1) (§16.9) | **Assigned** |
| 304 | `BP_TXID` — the transaction that paid the burn address (§9.5) | **Assigned** |
| 305 | `BP_AMOUNT` — the amount the out-proof proves, pXMR (§9.5) | **Assigned** |
| 306 | `BP_HEIGHT` — the block the burn is in (§9.5) | **Assigned** |
| 307 | `BP_PROOF` — Monero's `OutProofV2` string (§9.5) | **Assigned** |
| 308 | `BP_PERSONA` — the persona the proof's message names; the envelope's signer (§9.5) | **Assigned** |
| 309 | `BP_PURPOSE` — the purpose label in the message (§9.5) | **Assigned** |
| 310 | `AT_SUBJECT` — the persona an attestation is about (§9.2) | **Assigned** |
| 311 | `AT_AMOUNT` — what settled, pXMR (§9.2) | **Assigned** |
| 312 | `AT_RATING` — 1..=5 (§9.2) | **Assigned** |
| 313 | `AT_TS` — seconds since the epoch (§9.2) | **Assigned** |
| 314 | `AT_TXID` — the settling transaction, optional (§9.2) | **Assigned** |
| 315 | `AT_NOTE` — one sentence, optional, hazard-checked (§9.2) | **Assigned** |
| 316 | `AT_SIGNER` — the persona giving it; the envelope's signer (§9.2) | **Assigned** |
| 317 | `VO_SUBJECT` — the persona vouched for (§9.2) | **Assigned** |
| 318 | `VO_TS` — seconds since the epoch (§9.2) | **Assigned** |
| 319 | `VO_SIGNER` — the persona vouching; the envelope's signer (§9.2) | **Assigned** |
| 320 | `RN_MIN_BURN` — a listing's minimum burn for buyers, pXMR (§16.18, §9.5) | **Assigned** |
| 321 | `SLC_CLAIMANT` — the persona filing a slash claim (§17.5) | **Assigned** |
| 322+ | Unallocated | — |
...
| `contact.details` | What each side writes into the contact inbox, as a signed envelope under the persona it names: persona, outbox key, prekey bundle, the inbox and the role it was written for; a bare map, a version-1 body, or a signature under another key is refused (§16.9, §16.12) |
...
| `site.head` | A site head decodes, or is refused for §16.22's rules — version gate, the required share and 32-byte digest, the title bound, and the strict reader's edge (285) |
...
| `burn.proof` | A persona's burn proof opens only under the persona it names, and its shape — an amount, a block, a whole `OutProofV2`, a bounded purpose — is refused before any node is asked (§9.5) |
| `attestation.receipt` | A rated receipt opens only under its signer, rates within 1..=5, carries a time, and is never about the signer itself (§9.2) |
| `vouch.known` | A vouch opens only under its signer, carries a time, and is never for the signer itself (§9.2) |
