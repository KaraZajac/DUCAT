...
- **O1.** **Multisig — mechanically validated end to end; the remaining risk is not Monero's.** A 2-of-3 ceremony converged in 2 rounds and 134 s, and a funded bond was subsequently **seized by two parties without the third**, including key-image reconstruction from only the threshold count (`monero-spike/REPORT.md`). §17.2's deposit model is sound as specified. Three caveats stand: this exercised **wallet2's** multisig, not the FROSTLASS implementation §8.2 intends to ship, so the mechanism is proven and the code path is not; wallet2's own maintainers still label theirs experimental; and none of this speaks to whether an arbiter set behaves honestly, which is O8 and unaffected.
...
- **O21.** **Conformance suite exists, single-sourced.** 92 vectors in `vectors/v1/` now cover §18.9(1)–(7) including full per-profile transcripts, executable and language-neutral, with a runner proving the artifact matches the implementation. Still not closed, and for an unchanged reason: a vector set generated and validated by one implementation encodes that implementation's bugs as the specification. It closes when a second, independent client runs these files and disagrees. Remaining gaps are escrow and `fast/1` transcripts, which await `TXPROOF` and the escrow objects.
- **O22.** **An escrow participant who loses their device strands the escrow** (§4.3.3, §8.2). A Monero multisig share is not derivable from the wallet seed, and `monero-wallet-rpc` has no restore method for a multisig seed either — only the interactive CLI does, which is not reachable from a phone client. So a buyer who loses their device mid-escrow leaves every buyer-favourable outcome requiring the seller's signature, including a `RULING` in the buyer's favour, and nothing can compel it. Bounded today by escrow being off the default path, by the FUND-to-release window being short, and by the value being known before entering — none of which is a fix. Candidate directions, none evaluated: an arbiter-heavy composition for high-value escrows so two arbiter keys can execute a ruling without the counterparty; a client-side rule capping escrow value at what the user would accept losing to a dropped phone; or FROSTLASS (§8.2), whose share model may admit resharing where wallet2's does not. **This is the only place in the protocol where a lost device loses money rather than convenience**, which is the reason it earns an open problem rather than a caveat.
---
