...
**Draft 1.1.0-dev10 — Publications track (branch; 1.0.0-rc1 is the frozen line)**
...
**Attestations, restated for §9.2.** An `ATTESTATION` (object type 12) is a rated `RECEIPT`: the subject persona, the signer's persona, the settled amount, a rating from a closed set, and the receipt or transaction it stands on, signed by the signer under §18.3. It is issued after a settled deal into the subject's attestation record, whose writer key the subject holds (§4.3 already carries those keys in the backup). A reader weights each attestation by the signer's own verified burn or bond, so a review from an unburned persona is a sentence and not a score. Field numbers and vectors land with the object.
