...
**The ladder: nobody depends on an arbiter existing.** With no arbiter shared, the same entry point builds a **2-of-2 on mutual stakes**: the rider funds the fare *plus a margin* (a fifth, by default), and the release is a **split** — one co-signed transaction paying the fare to the driver and the margin home to a fresh per-ride refund subaddress the ceremony named at birth. The game theory replaces the judge: the fare is already beyond the rider's unilateral reach, so releasing costs nothing and returns their margin — sulking is strictly worse; the driver extorting burns the fare they earned. The driver stakes no capital, deliberately: their skin is the fare at risk, and a driver-side deposit would gatekeep exactly the people who start with nothing. What 2-of-2 gives up is named plainly: a lost phone with no backup strands the money (§4.3.3's multisig shares in the backup are the recovery), a genuine dispute has no partial ruling — only the split both agree to, or the burn — and at reservation-scale sums that trade gets worse, which is why an arbiter is preferred the moment one exists.
