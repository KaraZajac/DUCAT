...
- Monero multisig is multi-round and historically brittle; wallet-sync and key-exchange failures are common. The protocol must treat multisig setup as a fallible sub-state-machine with explicit timeouts and fund-recovery paths, not a single call.
...
0a. Instrument real Veilid private-route blob sizes (§15.3, O11). Now narrower than it was: §15.3.2 establishes that QR (2,953 B) and HCE (~1 KB per contact) carry `inline` mode comfortably, so this no longer gates the tap's *shape* — it decides whether `inline` mode survives on the tightest media and whether static tags need NTAG216. Still an afternoon, still worth doing first.
0c. Confirm the status of **Veilid issue #395** (importer deanonymization via hostile private routes, §15.10) against current veilid-core. The tap imports a counterparty-supplied route by design; if the side channel is live, `token` mode becomes mandatory rather than preferred and Phase 2 inherits a hard dependency on an upstream fix.
0b. Measure Veilid throughput at **block-sync volumes** (§8.7.1, O14). §8.7 assumes a wallet can pull full blocks over a private route using application-level messaging; Veilid is not a stream transport, and if bulk transfer is impractical, `relay/1`'s scanning half needs a different shape. Same cost, same class of unknown as 0a — and it gates Phase 1, not Phase 3.
...
- **O1.** Monero multisig fragility makes §8.2 the riskiest engineering in the protocol.
...
- **O11.** **Unmeasured:** Veilid route-blob size (§15.3). *Downgraded from blocking* — §15.3.2 establishes that QR and HCE both carry `inline` mode, so this decides media coverage and static-tag chip choice rather than Part II's shape.
...
- **O14.** **Veilid at sync volumes** (§8.7.1, Phase 0b): scanning means pulling block data over application-level messaging, not a stream transport. Whether that is practical on a phone is unmeasured, and `relay/1`'s scanning half depends on the answer.
...
- **O18.** **The conformance suite does not exist yet.** §18.9 specifies required coverage, but vectors must be authored alongside the first implementation — and a vector set validated by only one client validates nothing, since it encodes that client's bugs as the spec. The second implementation is what makes the first one's vectors trustworthy, which means §11's "many clients" goal stays unproven until there are two.
...
Which gives two sizes to design against: **`token` mode ≈ 190 bytes**, **`inline` mode ≈ 360–960 bytes**.
...
- **Hostile route blobs can deanonymize the reader.** The reader imports a Veilid private route supplied by an untrusted counterparty — which is exactly the shape of Veilid issue #395, where an adversary publishes an unreachable "evil" route, the importer's routing table liveness-pings its first hop directly, and the adversary correlates the pinging IP to the importer. A hostile terminal could therefore learn the payer's address at the moment of payment, defeating the network-layer guarantee at the one point it matters most. Tracked as an **upstream dependency**, not a DUCAT-layer bug; the proposed fix is privacy-domain tracking that suppresses direct pings to privately-discovered nodes. Clients SHOULD prefer `token` mode (§15.3.1) where DHT reachability permits, since fetching a route from the DHT is not the same act as importing one handed to you. Status must be confirmed against current veilid-core before Phase 2.
...
- **Outputs, not balance — the constraint that breaks a naive implementation.** A freshly-received Monero output, *including change*, is not spendable for ~10 blocks (≈20 min). Whether this is strictly a consensus rule or a wallet-enforced reorg-safety margin is debated, and eliminating it is live research (monero-project/research-lab #95) — but the practical effect is not in doubt, and a client cannot assume it away. A float held as a single output funds exactly one payment per lock interval: the second tap fails with a full balance showing on screen. This is not a corner case, it is the second ride. At load time the client MUST **pre-split the float into N spendable outputs** (default N ≈ 20, each sized around the user's typical transaction), and MUST re-split opportunistically as unlocked outputs are consumed, warning the user before the count reaches zero rather than at the curb.
