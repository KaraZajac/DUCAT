...
**How a receipt travels.** §9.2's `ATTESTATION` goes the same road. The signer sends it to its subject as a `ducat:attest/<hex>` link; the subject's client keeps it only if the envelope opens under the persona that sent it — a receipt handed on by anyone but its signer is discarded — and only if it is about one of the reader's own personas. When somebody asks for the record, the subject sends what it holds as `ducat:record/<hex>.<hex>…`, envelopes joined by dots, sixty-four at most; the reader keeps each one that opens and is about the sender, and drops the rest without comment. What the reader then says about the record is its own arithmetic: how many receipts it read, and — the number that means anything — how many distinct signers among them have a burn the reader verified itself, one voice per signer however many receipts that signer wrote, rated by the latest. A record is therefore never a score somebody else computed: it is envelopes, and every reader weighs them again.
