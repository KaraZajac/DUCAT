...
| Donate | payee | **open** | none | `TapStatic` (§15.9) — an address, not a relationship |
...
**Donate** is not a contact card and MUST NOT be built as one. A donation target is §15.9's `TapStatic` stance — reusable, public, receive-only, establishing no relationship — and it inherits §15.9's admitted limits verbatim: the address is reused (every donor is linkable to every other on the public ledger), and a wholly swapped target verifies. An implementation MUST state the reuse cost where the target is created, once and plainly.
...
3. **A driver claims the card.** Claim-once means exactly one driver wins — **the race is settled by the DHT, not by a matchmaker** (§16.12: one reply subkey, a second claimant has nowhere to write).
...
- **A receiver** MUST drop non-increasing counters (an old update replayed inside the ride), MUST render staleness as staleness — *"last seen 40 s ago"*, never a guessed position — and MUST treat the whole stream as the claim it is: spoofable input, drawn like the plate, checked against the window.
...
Single use is enforced by the shape of the record rather than by bookkeeping: the inbox has **exactly one** reply subkey, so a second claimant has nowhere to write. An issuer that has read a reply SHOULD delete the record, which turns a stale card into a clean failure instead of a silent one.
...
A stand's write key is `SHA-256("DUCAT-STAND-v0" ‖ 0x00 ‖ cell)` (§15.12). Everybody who can *find* a board therefore holds the key to every slot on it, and no design here changes that: **anyone can overwrite anything.** That is what having no operator means. Two narrower things are still worth establishing, and both notices carry them — `RENTAL_NOTICE` in fields 242–244, `HAIL_NOTICE` in 245–247.

**A notice says who wrote it.** Not who owns the slot; nobody owns a slot. Who authored the bytes. That is what makes substitution visible: copy a listing, swap in your own card, and it comes back as a *different author*, which a reader who has seen the original before can be told about. The signature is over `board ‖ 0x00 ‖ subkey(4, LE) ‖ 0x00 ‖ body`, under object type `BOARD_NOTICE`, where `body` is the notice re-encoded with these three fields removed. **The slot is inside the signature**, so a valid notice cannot be lifted onto another one — without that binding, an attacker holding the public write key could paper a whole cell with one signed listing and have it read as that person flooding the board.
...
**Writing one costs something.** A nonce such that `SHA-256("DUCAT-POW-v0" ‖ 0x00 ‖ signed ‖ 0x00 ‖ sig ‖ nonce(8, LE))` shows at least 20 leading zero bits. Proof of work is symmetric and 128 slots is not many, so this does not *prevent* a determined flood of one cell — no difficulty an honest phone could also pay would. What it does is stop flooding being free: a cell costs a minute or so and a region costs hours, repeated as notices expire.

The two are one mechanism. A notice missing either field is refused rather than treated as legacy — an accepted unsigned notice is the way to skip the work entirely, and a defence with an opt-out is not one. This is why both notices are version 2; a version-1 notice carries neither.
...
| 218–219 | `POSITION_REF` record key and stream key (§15.12) | **Assigned** (spec ahead of code) |
...
| `rental.listing` | A listing off a public board decodes, or is refused for §16.18's rules — the coarser cell, the place/vehicle split, the refused enumerations |
