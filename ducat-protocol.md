...
- **O18.** **The conformance suite exists but is single-sourced.** 88 vectors covering §18.9(1), (2 partial), (3), (5), (6) and (7) ship in `vectors/v1/`, executable and language-neutral, with a runner proving the artifact matches the implementation. **This does not close the problem.** A vector set generated and validated by one implementation encodes that implementation's bugs as the specification; it closes when a second, independent client runs these files and disagrees. §18.9(4) — full per-profile transcripts — remains uncovered and is blocked on the spec fixing field numbering for `TapPresent`, `FullOffer`, and `ACCEPT`: there is no wire object to build a transcript from yet. **That field numbering is now the single largest blocker in Part V.**
