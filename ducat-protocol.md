...
The honest costs, named: the arbiter must be online at the build (§17.8's governance problem wearing work clothes); the release spend waits out Monero's ~20-minute output maturity, so a short ride's payout lands after the parties part; and a 2-of-2 without the arbiter is deliberately not offered for rides — either party's lost phone or refusal would strand the fare forever, and Monero has no timelock to pre-sign an escape with. What the bond buys is exactly what it says: walking away now means abandoning staked money rather than shrugging, in both directions.
